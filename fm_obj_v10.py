...
Objective for Full Model implementation w/ fb-measure, accuracy and precision as metrics
...
def data_preprocessing(val_test_size,
    ...
    # Speichern der reversed_label in Dict, um sie später zurückkonvertieren zu können
    reversed_label = dict(zip(y, df['label'].values))
    # to_categorical: macht aus Klassenvektoren Binäre Klassenmatrix (https://keras.io/utils/#to_categorical)
    ...
    tokz = "fm_v10" + sep_ + "tokenizer.pickle"
    with open(tokz, 'rb') as handle:
        tokenizer = pickle.load(handle)

    # saving reversed label
    revlabel = "fm_v10" + sep_ + "reversed_label.pickle"
    ...
    print('\nNew Shapes of X_train, X_val, X_test...')
    print(X_train.shape)
    print(X_val.shape)
    print(X_test.shape)
...
def create_model(X_train, y_train, X_val, y_val, X_test, y_test, num_classes):
    ...
    model.load_weights("fm_v10" + sep_ + "model.hdf5")
