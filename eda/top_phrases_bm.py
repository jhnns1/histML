...
def data_preprocessing(val_test_size,
    ...
    # df['befund'].replace(to_replace='"\r"',
    #                      value='',
    #                      inplace=True,
    #                      regex=True)
    #
    ...
    if set_custom_n==True:
        ...
        # y = to_categorical(le.fit_transform(df['label'].values), num_classes=None)
    ...
    df = df[df['eindeutig']==True]

    # # print out für häufigste Label-Befunde-Kombi
    # df1 = df[df['count_befund'] > 50]
    # print(len(np.unique(df1['label'])))
    # print(len(df1))
    #
    # df_down = df1.groupby(['label','befund']).size().reset_index().rename(columns={0: 'count'})
    # df_down.to_csv("C:\\Users\\johannes.heck\\Desktop\\df_down.csv", sep="~", encoding='latin1')


    df1 = df[df['count_befund'] > 100]
    print(len(np.unique(df1['label'])))
    print(len(df1))
    ...
    df = df_100.groupby(['label']).size().reset_index().rename(columns={0: 'count'})
    print(len(np.unique(df['label'])))

    df2 = df_100.groupby(['befund']).size().reset_index().rename(columns={0: 'count'})
    print(len(np.unique(df2['befund'])))
    # df.to_csv("C:\\Users\\johannes.heck\\Desktop\\df.csv", sep="~", encoding='latin1')
    ...
    # X_safe = df
    # print("\nX_safe")
    # print(np.unique(X_safe['befund']))
    # print(np.unique(X_safe['label']))
    # # #
    # df.to_csv("C:\\Users\\johannes.heck\\Desktop\\df_imp.csv", sep="~", encoding='latin1')
    # # print(len(np.unique(df['label']))) # 487 verschiedene label
    #
    # print(df)
    ...
    # reversed word and labeling
    # print(np.argmax(y))
    # print(reversed_label.get(np.argmax(y)))
    my_texts = tokenizer.sequences_to_texts([[10,11,3]])
    # print(my_texts)
...
def create_model(X_train, y_train, X_val, y_val, X_test, y_test, num_classes):
    ...
    learning_rate_reduction = ReduceLROnPlateau(monitor='val_fbeta_score_',
                                              patience=5,
                                              verbose=1,
                                              factor=0.5,
                                              min_lr=0.0001)

    adam_ = Adam(lr=0.001,clipnorm=1.0)
    ...
    hist = model.fit(X_train,
                     ...
                     callbacks=[modelcheckpoint,tensorboard,earlystopping,metrics_,learning_rate_reduction])
    ...
    #
    # fb_b05_svm = fbeta_score_(y_test, pred_svm, beta=0.5)
    ...
    #
    # fb_b05_nb = fbeta_score_(y_test, pred_nb, beta=0.5)
