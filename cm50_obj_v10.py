...
Objective for Combined Model w/ rules threshold 50 implementation w/ fb-measure, accuracy and precision as metrics
...
def data_preprocessing(val_test_size,
    ...
    # hier werden Regeln mit Threshold 50 erstell, indem das Label mit der Sequenz von Einträgen kombiniert wird
    # (= LOW_label)
    # und anschliessend auf Eindeutigkeit geprüft wird
    # df2 beinhaltet dann alle Befund-Label-Kombinationen, die seltener als 50 mal vorkommen oder nicht eindeutig sind
    ...
    print("\nExporting rules...")
    # df_down enthält alle "Regeln" (Label-Befund-Kombination, die eindeutig sind und mehr als 50 Mal vorkommen)
    df_down = df1.groupby(['label', 'befund']).size().reset_index().rename(columns={0: 'count'})
    df_down.to_pickle("cm50_v10" + sep_ + "rules50.pkl")
    ...
    tokz = "cm50_v10" + sep_ + "tokenizer.pickle"
    with open(tokz, 'rb') as handle:
        tokenizer = pickle.load(handle)

    # saving reversed label
    revlabel = "cm50_v10" + sep_ + "reversed_label.pickle"
...
def create_model(X_train, y_train, X_val, y_val, X_test, y_test, num_classes):
    ...
    model.load_weights("cm50_v10" + sep_ + "model.hdf5")
    ...
    df_rules = pd.read_pickle("cm50_v10" + sep_ + "rules50.pkl")
