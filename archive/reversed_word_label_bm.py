...
def data_preprocessing(val_test_size,
    ...
    # df['label'][df['counts'] < 6] = 'minority_class'
    ...
    for i in range(0,10):
        print(reversed_label.get(np.argmax(y[i])))
    # print(np.argmax(y))
    # print(reversed_label.get(np.argmax(y)))

    my_texts = tokenizer.sequences_to_texts(X_tok[:10])
    # my_texts = list(map(sequence_to_text, X_tok[:10]))
    print(my_texts)
    ...
    #     X_train = np.load("C:\\Users\\johannes.heck\\Desktop\\smote\\smoteenn_X_train.csv")
    #     y_train = np.load("C:\\Users\\johannes.heck\\Desktop\\smote\\smoteenn_y_train.csv")
    # else:
    #     print('\nOversampling + Undersampling combined w/ SMOTEENN...')
    #     sm = SMOTEENN(sampling_strategy='not majority', random_state=13)
    #     X_train, y_train = sm.fit_sample(X_train, y_train)
    #
    #     y_train = y_train.round()           # artificial points to categorical, might be an issue!
    #
    #     np.save("C:\\Users\\johannes.heck\\Desktop\\smote\\smoteenn_X_train.csv",X_train)
    #     np.save("C:\\Users\\johannes.heck\\Desktop\\smote\\smoteenn_y_train.csv",y_train)
    #
    # print(X_train.shape)
    # print(X_val.shape)
    # print(X_test.shape)
