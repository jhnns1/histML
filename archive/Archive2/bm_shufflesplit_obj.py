...
def data_preprocessing(test_size,
                       max_len,
...
def create_model(X_train, y_train, X_test, y_test, num_classes):
    ...
    model.add(GRU(128,
                  kernel_regularizer=regularizers.l2(0.3898),
                  dropout=0.5159,
                  recurrent_dropout=0.2554,
                  activation='tanh'))
    ...
    rs = ShuffleSplit(n_splits=35, test_size=0.3, random_state=0)
    i = 0
    patience = 0
    hist = []
    earlystopping_crit = 100
    for train_index, val_index in rs.split(X_train,y_train):
        print('\nActual epoch: {}' .format(i))
        res = model.fit(X_train[train_index],
                              y_train[train_index],
                              ...
                              callbacks=[modelcheckpoint,metrics_])
        # hist.append(res.history)
        if earlystopping_crit > round(res.history['val_loss'][0],2):
            earlystopping_crit = round(res.history['val_loss'][0],2)
            patience = 0
        elif patience == 10:
            model.load_weights(filepath=output_dir + 'best_weights.hdf5')
            break
        else:
            patience += 1
        i +=1
